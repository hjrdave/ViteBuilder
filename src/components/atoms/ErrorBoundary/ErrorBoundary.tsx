...
interface Props {
    ...
    fallback?: 'button' | 'control' | boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null;
    pathname?: string;
}
...
export default function ErrorBoundary({ fallback, children, pathname }: Props) {
    ...
    return (
        <>
            <ErrorBoundaryComp 
              key={pathname} 
              {...{ fallback: () => (<BrokenCompFallback variant={(typeof fallback === 'string') ? fallback as any : fallback} />), children: children }}
            />
        </>
    )
};